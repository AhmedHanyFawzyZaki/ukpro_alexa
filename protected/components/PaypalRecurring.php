 ...
 *
 */
class PaypalRecurring extends CComponent{
...
//	$SandboxFlag = true;
	...
	*/
	...
	'--------------------------------------------------------------------------------------------------------------------------------------------
	*/
	public function CallShortcutExpressCheckout( $paymentAmount, $paymentType, $description)
	{
		//------------------------------------------------------------------------------------------------------------------------------------
	...
	'--------------------------------------------------------------------------------------------------------------------------------------------
	*/
	public function CallMarkExpressCheckout( $paymentAmount, $paymentType, $returnUrl,
	...
	*/
	...
	'--------------------------------------------------------------------------------------------------------------------------------------------
	*/
	public function ConfirmPayment( $token, $paymentType, $payerID, $FinalPaymentAmt )
	{
		...
		$nvpstr .= '&CURRENCYCODE=' . $this->currencyCodeType . '&IPADDRESS=' . $serverName;
	...
	public function CreateRecurringPaymentsProfile($token, $shipToName, $shipToStreet, $shipToCity, $shipToState, $shipToZip, $shipToCountry, $description, $period, $frequency, $amount , $days_num)
	{
		//'--------------------------------------------------------------
		//' At this point, the buyer has completed authorizing the payment
		//' at PayPal.  The function will call PayPal to obtain the details
...
******/
		...
		$dateRecurring = date('Y-m-d H:i:s', strtotime(' +'.$days_num.' day'));
		$nvpstr.="&PROFILESTARTDATE=".urlencode($dateRecurring);
	...
	/*
	   '-------------------------------------------------------------------------------------------------------------------------------------------
	   ' Purpose: 	get recurring prifle information for the GetRecurringPaymentsProfileDetails API Call.
	   '
	   ' Inputs:
	   '		PROFILEID:	The recurring profile if used by PayPal to track the transactions from a given account.
	   ' Returns:
	   '		The NVP Collection object of the GetRecurringPaymentsProfileDetails Call Response.
	   '--------------------------------------------------------------------------------------------------------------------------------------------
	*/
	...
	*/
	...
	//	global $API_Endpoint, $version, $apiUsername, $apiPassword, $apiSignature;
