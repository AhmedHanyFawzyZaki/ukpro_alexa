...
class WebsiteController extends AdminController
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Website']))
		{
                    ...
                    $uploadedFile=CUploadedFile::getInstance($model,'fav_icon');

                    if(! empty ($uploadedFile)){
                        $fileName = "{$rnd}-{$uploadedFile}";  // random number + file name
                        $model->fav_icon = $fileName;
                        $uploadedFile->saveAs(Yii::app()->basePath.'/../media/website/'.$fileName);
                    }
			if($model->save())
	...
	public function actionUpdate($id)
	{
		...
		{
                    if( $model->fav_icon != ''){
                        $_POST['Website']['fav_icon'] = $model->fav_icon;
                    }
			$model->attributes=$_POST['Website'];
                        $uploadedFile=CUploadedFile::getInstance($model,'fav_icon');
                        if(! empty ($uploadedFile)){
                            ...
                            $uploadedFile->saveAs(Yii::app()->basePath.'/../media/website/'.$model->fav_icon);
			}
			if($model->save())
				$this->redirect(array('view','id'=>$model->id));
		}
